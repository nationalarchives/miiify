...
let get_annotation ctx request =
  ...
  match hash with
  | Some hash -> (
      match get_if_none_match request with
...
let get_annotation_pages ctx request =
  ...
  let prefer = get_prefer request ctx.config.container_representation in
...
let delete_container ctx request =
  ...
  match hash with
  | Some hash -> (
      match get_if_match request with
...
let post_container ctx request =
  ...
  | Some host -> (
      ...
      Data.post_container ~data:body ~id:[ get_id request; "main" ] ~host
      |> fun obj ->
      match obj with
...
let configure () =
  parse_cmdline ();
  let data = match !config_file with "" -> "{}" | fname -> read_file fname in
