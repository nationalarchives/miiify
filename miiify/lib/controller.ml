...
let get_annotation_collection ~page_limit ~db ~id ~target =
  let* total = Model.total ~db ~container_id:id in
  Model.get_annotations ~db ~container_id:id ~offset:0
    ~length:page_limit ~target
  >|= Container.collection ~total ~target ~limit:page_limit

let get_annotation_page ~page_limit ~db ~id ~page ~target =
